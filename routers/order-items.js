...
router.get(`/`, async (req, res) => {
    const orderList = await OrderItem.find();//find all of the product in order
...
router.get(`/:id`, async (req, res) => {     //find 1 order-item by id
...
router.post(`/`, async (req, res) => {      //create order item
    ...
    if(!orderItems) {
        return res.status(404).send('the order-items cannot be created');
    }
    
    res.send(orderItems);
})

router.put('/:id', async (req, res) => {        //Update order item status
    try {
        const { id } = req.params;
...
router.delete('/:id', (req, res) => {
    OrderItem.findByIdAndDelete(req.params.id).then(orderItems => {            //delete a order by id
        if(orderItems) {
